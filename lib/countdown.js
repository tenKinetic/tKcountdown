...
tkCountdown.begin = function(opts) {
  ...
  if (typeof opts.mode !== 'undefined') {
    ...
    else if (opts.mode == 'airport') {

      tkCountdown.element = document.querySelector('.tk-countdown.airport')
  ...
  } else if (tkCountdown.mode == 'airport') {
...
tkCountdown.render = function() {
  ...
  else if (tkCountdown.mode == 'airport') tkCountdown.renderAirport()
}
...
tkCountdown.renderAirport = function() {
  ...
  if (parseInt(remainingTimeSpan / 1000) !== tkCountdown.currentSecondsTimeSpan) {
    var display = tkCountdown.display(remainingTimeSpan)
    // tkCountdown.weeksLabel.innerText = display.weeks
    // tkCountdown.daysLabel.innerText = display.days
    // tkCountdown.hoursLabel.innerText = display.hours
    // tkCountdown.minutesLabel.innerText = display.minutes
    // tkCountdown.secondsLabel.innerText = display.seconds

    // TODO: run the flip
    console.log('flip')

    if (tkCountdown.currentDisplay) {
      // flip those values that have changed
